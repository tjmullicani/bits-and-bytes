...
The next caveat I was warned about is that y-cruncher will leave behind files on drives used during the computation. Not all of these files are required to be backed up, and the unneeded ones should be removed before starting a backup. In order to ensure the checkpoint file and files on disk are not overwritten, you must stop the y-cruncher program during a backup. Since there wasn't an existing way to automatically remove the unneeded files, I wrote a simple Bash [script]({% post_url 2019-06-27-pi-world-record-checkpoint-remove-files %}) to delete files that were not required by the y-cruncher checkpoint file.

After I have deleted the unneeded files from the drives, I have an ansible script which connects to the Dell R720xd running Windows Server 2016. The script modifies and executes a Veeam direct file-to-tape backup job on the fly. The script is also able to provide an estimated completion time, since Veeam doesn't provide a time estimate natively. I will make these scripts publicly available in the coming weeks.
